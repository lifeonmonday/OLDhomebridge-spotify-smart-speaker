...
class SpotifySmartSpeakerPlatform {
  ...
  configureAccessory(accessory) {
    // Unused for external accessories
  }
  ...
  async discoverDevices() {
    try {
      ...
      // Publish as an External Accessory so HomeKit gets proper Television/Speaker media controls
      this.api.publishExternalAccessories(PLUGIN_NAME, [accessory]);
    } catch (error) {
      this.log.error('Failed to initialize Spotify device:', error.message);
    }
  }
}
...
class SpotifySmartSpeakerAccessory {
  constructor(platform, accessory) {
    ...
    // Television service unlocks media widgets in Apple Home
    ...
    // Poll Spotify playback every 5s
  ...
  async setTargetMediaState(value) {
    ...
    try {
      ...
      this.platform.log.error('Failed to change playback state:', error.message);
    }
  }
  ...
  async refreshAccessTokenIfNeeded() {
    try {
      ...
      this.platform.log.error('Error refreshing token:', error.message);
    }
  }
}